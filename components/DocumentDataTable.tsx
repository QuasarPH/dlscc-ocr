...
import { UploadedFile, DocumentCategory, ExtractedData, TableColumn } from '../types';
...
const DocumentDataTable = <T extends ExtractedData>({ category, files, columns }: DocumentDataTableProps<T>) => {
  if (!files || files.length === 0) {
    return null; 
  }

  const getDisplayValue = (value: any, type?: 'string' | 'number' | 'date' | 'boolean'): string | number | React.ReactNode => {
    if (value === null || typeof value === 'undefined') return <span className="text-gray-400 italic">N/A</span>;

    switch (type) {
      case 'date':
        return formatDate(String(value));
      case 'number':
        return typeof value === 'number' ? value.toLocaleString() : <span className="text-gray-400 italic">N/A</span>;
      case 'boolean':
        return value === true ? <span className="text-green-700 font-semibold">Yes</span> : (value === false ? <span className="text-red-700 font-semibold">No</span> : <span className="text-gray-400 italic">N/A</span>);
      default:
        return String(value);
    }
  };
  ...
  const formatValueForCsv = (value: any, type?: 'string' | 'number' | 'date' | 'boolean'): string => {
  ...
  const handleDownloadCsv = () => {
    ...
    const csvHeader = ['File Name', ...columns.map(col => col.label)].map(escapeCsvValue).join(',');
    
    const csvRows = validFiles.map(file => {
      const rowData = [
        file.file.name, 
        ...columns.map(col => {
          const value = file.extractedData ? (file.extractedData as T)[col.key] : null;
    ...
    const safeCategoryName = category.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
  ...
  if (validFiles.length === 0) {
    return (
        <div className="mt-4 p-4 bg-gray-100 rounded-md shadow">
             <p className="text-gray-600">No data has been successfully extracted for {category} documents yet.</p>
        </div>
    );
  }
  ...
  return (
    <div className="my-8 p-2 bg-white shadow-xl rounded-lg animate-fadeIn">
      <div className="flex justify-between items-center px-4 py-3 bg-indigo-50 rounded-t-lg">
        <h3 className="text-2xl font-semibold text-indigo-700">{category} - Extracted Data</h3>
        <button
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider sticky left-0 bg-gray-100 z-10 min-w-[150px] max-w-[250px] truncate">File Name</th>
              {columns.map((col) => (
                <th
                  key={String(col.key)}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {validFiles.map((file) => (
              <tr key={file.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-800 sticky left-0 bg-white hover:bg-gray-50 z-10 min-w-[150px] max-w-[250px] truncate" title={file.file.name}>
                    {file.file.name}
                </td>
                {columns.map((col) => (
                  <td
                  ...
                  >
                    {getDisplayValue(file.extractedData ? (file.extractedData as T)[col.key] : null, col.type)}
                  </td>
                ))}
              </tr>
            ))}
       ...
       {validFiles.length === 0 && ( // This condition is now technically redundant due to the early return but kept for logical flow.
            <p className="p-4 text-sm text-gray-500">No data to display for this category.</p>
        )}
    </div>
  );
};
